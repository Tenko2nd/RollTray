...
void loop() {
  ...
  if (!calibrageFait) {
    ...
    if ((checkTimerCard1() || checkTimerCard2() || checkTimerCard3() || checkTimerCard4()) && value_button == etat_bp_appuye && !retour_en_cours && envoi_effectue) {
    ...
    else if (livraison_faite && value_button == etat_bp_relache && !retour_en_cours) {
    ...
    else if (value_card == 0 && retour_en_cours) {
      current_affichage = PLATEAU_ARRIVE;
      if (current_affichage != old_affichage) {
        afficher_plateau_arrive();
        old_affichage = current_affichage;
