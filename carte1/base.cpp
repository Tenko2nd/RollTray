...
char forbidden_keys[11] = {'4', '5', '6', 'B', '7', '8', '9', 'C', '0', 'F', 'D' };
char num_keys[3] = {'1', '2', '3' };
...
void ecran_demarrage() {
  ...
  int16_t x_return = (display.width() - (start_msg.length() * 6 * 2)) / 2;
  int16_t y_return = (display.height() / 2) - (2 * 8 / 2);
  display.setCursor(x_return, y_return);
  display.print(start_msg);
}
