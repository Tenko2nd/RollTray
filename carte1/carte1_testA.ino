...
void setup() {
  ...
  Serial.print("Demarrage Carte 1 - Version "); Serial.println(currentFirmwareVersion_OTA);
  ...
  if (WiFi.isConnected()) {
    ...
    int port = atoi(mqtt_port_str);  // Convertir la chaîne du port en entier
...
void loop() {
  ...
  if (!calibrageFait) {
    ...
    if (current_key != 0) {
      ...
      else if ((is_numerous(current_key)) && current_key != bouton_envoi && conveyorToBase && !retour_en_cours) { // Avant : if !is_forbidden(current_key)
      ...
      // --- Si un bouton interdit est appuyé (autre que 1, 2 ou 3) dans les autres cas que cités précédemment ---
      else if (is_forbidden(current_key) || (is_numerous(current_key)) || (current_key == bouton_stop) || (current_key == bouton_stop && !retour_en_cours)) { // Avant : juste "else"
    ...
    if ((value_card == 0 || current_key == bouton_stop) && retour_en_cours) {
      current_affichage = PLATEAU_ARRIVE;
      if (current_affichage != old_affichage) {
        afficher_plateau_arrive();
        old_affichage = current_affichage;
